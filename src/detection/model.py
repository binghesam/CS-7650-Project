...
class BertForMultitask(BertPreTrainedModel):
    ...
    def __init__(self, config, cls_num_labels=2, tok_num_labels=2, tok2id=None):
        ...
        # self.init_bert_weights the function of bert pretrained model
        # self.apply--->> used to the every submodeul(like linear )
...
# from the paper, this should be a weak classifer? in mymory
# thus, we just use a nn.Module to set it! by bing to check
class ConcatCombine(nn.Module):
    def __init__(self, hidden_size, feature_size, out_size, layers,
        ...
        # by checking, layers can only be 1 or 2 in the setting
        # find the key part: the hidden size and feature size?--- related to the paper setting?
        # bing: it should be related to the equation 1 for the setting! with hidden_size + feature_size
        if layers == 1:
    ...
    def forward(self, hidden, features, categories=None):
        ...
        # TODO: to check by bing to see the cat function for many possible usage
        # why dim is -1
        return self.out(torch.cat((hidden, features), dim=-1))


# it seems that this is not used at all with some error, I comment it for a whicle
class AddCombine(nn.Module):
    def __init__(self, hidden_dim, feat_dim, layers, dropout_prob, small=False,
        ...
        if pre_enrich:
            # TODO: it seems the feature_size is not queried and used
            # comment to avoid the error for the checking
            assert False
            # self.enricher = nn.Linear(feature_size, feature_size)
        else:
            self.enricher = None
    ...
    def forward(self, hidden, feat, categories=None):
        if self.include_categories:
            categories = categories.unsqueeze(1)
            # TODO: check the existance and usage of features?
            # it seems that it is never used by the default concat setting!
            categories = categories.repeat(1, features.shape[1], 1)
            if self.add_category_emb:
                features = features + categories
            else:
...
class BertForMultitaskWithFeaturesOnTop(BertPreTrainedModel):
    ...
    def __init__(self, config, cls_num_labels=2, tok_num_labels=2, tok2id=None):
        ...
        if ARGS.extra_features_method == 'concat':
            ...
            # it seems that there are errors and I comment it without any usage
            # check it later by bing
            assert False
            pass
        ...
        # TODO: check the usage of these classifiers? why, we already have the token class
        self.cls_dropout = nn.Dropout(config.hidden_dropout_prob)
        self.cls_classifier = nn.Linear(config.hidden_size, cls_num_labels)
    ...
    def forward(self, input_ids, token_type_ids=None, attention_mask=None,
        ...
        # var.detach().cpu().numpy(), traditional usage to change tensor to
        # cpu environment for the related programming and setting!
        # TODO: why on the cpu mode? any reason?
        ...
        # get the bert embedding by the weights-- through it
        # we get the wordembedding
        sequence_output, pooled_output = self.bert(
            input_ids, token_type_ids, attention_mask, output_all_encoded_layers=False)
        ...
        # TODO: check the -1, [1] processing later by bing
        if ARGS.category_emb:
            categories = self.category_embeddings(
                categories.max(-1)[1].type(
                    'torch.cuda.LongTensor' if CUDA else 'torch.LongTensor'))
        # with each word's embedding, we can do the token setting (b in paper) + features (e) in paper
