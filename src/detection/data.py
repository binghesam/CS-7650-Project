...
def get_examples(data_path, tok2id, max_seq_len,
    ...
    for i, (line) in enumerate(tqdm(open(data_path, encoding="utf-8"))):
        ...
        try:
            ...
            pre_ids = pad([tok2id[x] for x in pre_toks], 0)  # not by zero, but [PAD] format
...
def get_dataloader(data_path, tok2id, batch_size,
    ...
    # if the data is prepared before, we can speed it up
    if pickle_path is not None and os.path.exists(pickle_path):
        print("use the preivous data")  # judge by bing
    ...
    # the basic data setting: by list of list to set the related processing time
